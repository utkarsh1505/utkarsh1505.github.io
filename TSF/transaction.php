...
$ids=$_GET['idtransfer'];
$showquery="select * from `records` where ID=($ids) ";
...
if(isset($_POST['submit']))
{
    ...
    $ids=$_GET['idtransfer'];
    $senderquery="select * from `records` where ID=$ids ";
    ...
    //receiverquery
    $receiverquery="select * from `records` where Email='$Receiver_email' ";
    ...
    if($arrdata['Amount'] >= $Sender)
    {
      ...
      $increase_receiver=$receiver_arr['Amount'] + $Sender;
       $query="UPDATE `records` SET `ID`=$ids,`Amount`= $decrease_sender  where `ID`=$ids ";
       $rec_query="UPDATE`records` SET `ID`=$id_receiver,`Amount`= $increase_receiver where  `ID`=$id_receiver ";
